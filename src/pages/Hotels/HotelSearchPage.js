...
const HotelSearchPage = () => {
  ...
  const handleSearch = async () => {
    ...
    if (!searchParams.destination || !searchParams.checkIn || !searchParams.checkOut) {
      ...
      if (!searchParams.checkIn) missingFields.push('check-in date');
      if (!searchParams.checkOut) missingFields.push('check-out date');
    ...
    // Validate check-in is today or in the future
    ...
    console.log('Room details:', searchParams.roomsDetails);
    ...
    try {
      ...
      // Flatten all children ages from all rooms
      ...
      console.log('Search payload to send:', searchPayload);
      console.log('API URL:', `${API_BASE_URL}/hotels/search`);
      ...
      setError(`Failed to search hotels: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDestinationSelect = (destination) => {
    // Set default dates if not already set (tomorrow and day after)
    ...
    setSearchParams(prev => ({
      ...prev,
      destination: destination.name, // Use name for search query
      checkIn: prev.checkIn || tomorrow,
      checkOut: prev.checkOut || dayAfter,
      page: 1
    }));
                ...
                <Grid item xs={12} sm={6} md={4}>
                  <Box>
                    ...
                    <Autocomplete
                      ...
                      value={
                        googleLoaded
                          ? placePredictions.find(p => p.description === searchParams.destination) || (searchParams.destination || null)
                          : destinations.find(d => d.id === searchParams.destination) || (searchParams.destination || null)
                      }
                      inputValue={placeQuery}
                      onInputChange={(e, value) => {
                        setPlaceQuery(value);
                        // For freeSolo, update destination as user types
                      ...
                      onChange={(e, value) => {
                        ...
                        // If it's a string (freeSolo input)
                        if (typeof value === 'string') {
                          setSearchParams(prev => ({ ...prev, destination: value }));
                          setPlaceQuery(value);
                        }
                        // If it's a place object from Google Places
                        else if (value.place_id) {
                          setSearchParams(prev => ({ ...prev, destination: value.description }));
                          setPlaceQuery(value.description);
                        }
                        // If it's a destination object
                        else {
                          setSearchParams(prev => ({ ...prev, destination: value.name || value.id }));
                          setPlaceQuery(value.name || value.id);
                        }
                      }}
                      renderInput={(params) => (
                        <TextField
                    ...
                    <Box sx={{ p: 2.5 }}>
                      ...
                      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
                        ...
                        {destination.priceFrom && (
                          <Chip
                            label={`From KES ${destination.priceFrom.toLocaleString()}`}
                            size="small"
                            sx={{
          ...
          <Container maxWidth="lg" sx={{ py: 4 }}>
            ...
            <Box sx={{
              ...
              <Typography variant="body2" sx={{ fontWeight: 600, color: '#002d68' }}>
                All taxes and fees included | {totalResults} results
              </Typography>
            ...
            <Stack spacing={2}>
              {searchResults.map((hotel) => (
                ...
                >
                  ...
                  <CardMedia
                    component="img"
                    image={hotel.images?.[0]?.path || 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=400'}
                    alt={hotel.name}
                    sx={{
                      width: { xs: '100%', sm: '180px', md: '200px' },
                      height: { xs: '180px', sm: '140px', md: '160px' },
                      objectFit: 'cover',
                      flexShrink: 0
                    }}
                  />
                  ...
                  <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', py: { xs: 1.5, md: 2 }, px: { xs: 1.5, md: 2 } }}>
                    <Box>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 0.5 }}>
                        ...
                        >
                          {favorites.includes(hotel.code) ? (
                            <FavoriteIcon sx={{ color: '#ff0080', fontSize: 20 }} />
                          ) : (
                      ...
                      {/* Tags Row */}
                      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 0.5, alignItems: 'center' }}>
                        {/* Rating */}
                        {hotel.reviews?.averageRating && (
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                            <StarIcon sx={{ fontSize: 16, color: '#ffa726' }} />
                            <Typography variant="caption" sx={{ fontWeight: 700 }}>
                              {hotel.reviews.averageRating.toFixed(1)}/5
                            </Typography>
                          </Box>
                        )}
                      ...
                      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 0.5 }}>
                        {hotel.facilities && hotel.facilities.slice(0, 2).map((facility, idx) => (
                          <Chip
                            key={idx}
                            label={facility}
                            size="small"
